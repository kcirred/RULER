...
parser.add_argument(
    ...
    type=str,
    default="llama_1b",
    help="provide the variant such as llama_1b, mamba_9.8b",
)
...
if (
    args.server_type == "hf"
    or args.server_type == "gemini"
    or args.server_type == "fms"
):
    args.threads = 1
...
class Hook4ssm_states(torch.nn.Module):
    ...
    def __call__(self, mod, args, kwargs):
        ...
        if tok_id == self.calib_on_nth_token:
            self.calib_scale = torch.tensor(
                max(self.scales), device=ssm_states_lay_i.device
            )
        ...
        tar_dtype = (
            torch.float8_e4m3fn
            if "fp8" in self.quant_opt
            else torch.int8
            if "int" in self.quant_opt
            else org_dtype
        )
        ...
        if scale is not None:
            if tar_dtype in [torch.float8_e4m3fn, torch.float8_e5m2]:
                ssm_states_lay_i = (
                    ...
                    .clamp(min=qmin, max=qmax)
                    .to(tar_dtype)
                    .to(org_dtype)
                )
            elif tar_dtype in [torch.int8]:
                ssm_states_lay_i = (
                    (ssm_states_lay_i / scale).round().clamp(min=qmin, max=qmax)
                )
...
def main():
    ...
    if args.hook:
        ...
        for n, m in llm.model.named_modules():
            if isinstance(
                m, BambaMixer
            ):  # and quant_opt is not None:  # m.layer_idx not in layers_to_skip:
